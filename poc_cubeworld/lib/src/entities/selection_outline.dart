...
///
/// Why cuboids and not a line geometry: a `LineSegmentsGeometry` node never
/// reaches the screen in this build.
///
/// Why a depth bias: a stick on the edge of a block that sits flush with its
/// neighbours lies half inside them, and the depth test ate those halves
/// (a block in a floor lost eleven of its twelve edges). The sticks are drawn
...
class SelectionOutline {
  SelectionOutline() {
    ...
    for (var axis = 0; axis < 3; axis++) {
      for (var corner = 0; corner < 4; corner++) {
        ...
        size[axis] = 1.0;
        final n = GodotCamera.primitiveNode(Mesh(CuboidGeometry(size), stick), castsShadows: false);
        _sticks.add(n);
        node.add(n);
      }
    }
  ...
  /// The box the skeleton was last fitted to, for the probes.
  CollisionBox? box;
  ...
  void show(CollisionBox b) {
    ...
    for (var axis = 0; axis < 3; axis++) {
      for (var corner = 0; corner < 4; corner++) {
        ...
        scale[axis] = span[axis] + 2 * gap + thickness; // corner to corner, closing the joints
        _sticks[axis * 4 + corner]
          ..position = centre
          ..scale = scale;
      }
    }
