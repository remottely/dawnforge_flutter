...
| VR1 The relayout — the kit moves under `packages/voxel_game/`, the app leaves the workspace | **done** 2026-09-22: analyze clean in both trees · 194 + 32 + 168 + 10 + 4 = **408 tests** · no hosted package changed version in either lock (the kit's lost `flutter_soloud`/`hooks` as *direct* deps, the app's lost `test` and the 26 packages only `voxel_engine`'s dev deps pulled) · the three probe logs identical to VR0's after the filter · a `--screenshot` of the relaid build shows the world, water, sky and HUD | analyze clean in both trees, the same test count, no hosted package changed version, probe logs match `docs/baseline/`, `flutter run -d macos` plays |
